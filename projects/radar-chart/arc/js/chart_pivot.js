async function drawRadar() {
    ...
    // 1. Access data
    const pathToData = './data/radar_pivot.csv';
    let dataset = await d3.dsv(";", pathToData);

    // group the data
    let skaters = Array.from(
    ...
    const metrics = [
        ...
        "StSq3",
        "CCoSp4",
        // 'Леонид СМИРНОВ', 'Алексей СУЧКОВ', 'Пётр КИПРУШЕВ',
        // 'Галина ИСТОМИНА', 'Анна ПОЛИКАРПОВА', 'Эльвира СЕРЕБРЯКОВА',
        // 'Святослав БАБЕНКО'
    ];
    ...
    const drawLine = (skater) => {
        const lineGenerator = d3.lineRadial()
            .angle((metric, i) => i * ((Math.PI * 2) / metrics.length))
            .radius((metric, i) => metricScales[i](+skater.value[i][metric] || 0))
    ...
    const updateChart = () => {
        title.text(judgeAccessor(skaters[activeJudgeIndex].value))
        drawLine(skaters[activeJudgeIndex]);
    }
    updateChart();
}
