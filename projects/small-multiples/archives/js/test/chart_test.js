...
async function drawChart() {
  ...
  let metrics = Array.from(
    d3.group(dataset, nameAccessor), ([key, value]) => ({ key, value })
  );
  // console.log(metrics);
  ...
  const bounds = wrapper
    .attr('class', d => d.value[0].name_rus)
  ...
  metrics.forEach(function (d, i) {
    yScale.domain([0, d3.max(d.value, function (c) {
      // console.log(d3.max(d.value, c=>+c.scores_of_panel));
    ...
    d3.select('svg.' + d.value[0].name_rus)
      ...
      .attr('class', 'circles')
      .append('circle')
      .attr('cx', function (c) {       console.log(c)
