//добавить цветовые сегменты по одинарным прыжкам, каскадам, вращениям и дорожкам
//показывать элементы по сегментно, а не по хронологии элементов в проргамме

//цветовой градиент легенды

async function drawChart() {
  const pathToData = './data/chart_ru_fs_w.csv';
  ...
  svg.append("text")
    ...
    .attr("x", 0)
    .text(function (d) {
      // console.log(d.value[0].name_eng);
      return (d.value[0].name_eng + ' - ' + d.value[0].total_element_score)
    })
    .style('transform', `translate(${dimensions.margin.left}px,
        ${dimensions.margin.top}px)`);
  ...
  metrics.forEach(function (d, i) {
    ...
    let myCustomColorScale = d3.interpolate('cyan', 'red');
    let myCustomColorScale2 = d3.interpolateHsl('cyan', 'red');

    const colorScale = d3.scaleSequential()
      .domain(d3.extent(d.value, c => +c.grade_of_execution))
      // .interpolator(d3.interpolateSpectral);
      .interpolator(myCustomColorScale);
    // .range(['#F79CD4', '#0FCFC0']);
    // .range(['#A50026', '#354B99']);
    // console.log(d);
